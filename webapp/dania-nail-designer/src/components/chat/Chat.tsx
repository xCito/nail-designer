...
export function Chat() {
  const [chatConvo, setChatConvo] = useState<ChatItem[]>(() => [CONVERSATION[START]]);
  ...
  const onOptionClick = (qIndex: number, selectedOption: ChatOption) => {
    ...
    updatedChat[qIndex].selected = selectedOption.label;

    // Insert next question and delete everything after inserted index
    updatedChat.splice(qIndex + 1, updatedChat.length, CONVERSATION[selectedOption.nextId]);
